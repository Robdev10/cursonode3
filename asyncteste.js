...
async function buscarUsuarioPeloID(id) {
    try {
        ...
        return usuario; // Retorna as informações do usuário
    } catch (error) {
        throw error; // Lança o erro novamente para que seja tratado por quem chamou a função
    }
}

// Chamando a função assíncrona para buscar um usuário pelo ID
buscarUsuarioPeloID(2)
    .then(usuario => {
